...
function processMessage(data, { display, audio }) {
    ...
    // Set display rotation if specified
    if (data.rotation !== undefined) {
        display.setRotation(data.rotation);
    }
    ...
    if (data.message) {
        ...
        if (data.sound) {
            audio.playSound(data.sound); // Use the specific sound requested
        } else {
            audio.playSound('message'); // Default notification sound
        }
...
function processCommand(data, { display, audio, sensors, client }) {
    switch (data.action) {
        case 'get_sensors':
            // Immediately publish sensor data
            if (sensors) {
