...
function displayMessage(message, color = config.display.defaultColor) {
    ...
    try {
        ...
        // Set rotation before showing message
        senseHat.setRotation(savedRotation);

        // The key step: Create the scrolling text without modifying rotation
        senseHat.showMessage(message, 0.1, color);

        // Force rotation back to what it should be
        setTimeout(() => {
            senseHat.setRotation(savedRotation);
        }, 100); // Small delay to ensure the showMessage has started
