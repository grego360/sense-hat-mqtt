...
// Variable to store joystick process
let joystickProcess;
        ...
        outputLines.forEach(line => {
            try {
                ...
                if (line.includes('"action"') && line.includes('"direction"')) {
                    ...
                    // Publish to MQTT
                    if (mqttClient) {
                        console.log('Publishing joystick event to MQTT:', event);
                        mqttClient.publish(config.mqtt.topics.joystick, JSON.stringify(event));
                    } else {
                        console.error('Cannot publish joystick event: mqttClient is not initialized');
                    }
