...
let lastJoystickEvent = {};
let joystickDebounceTime = 300; // milliseconds
...
function init(displayInstance, mqttClientInstance) {
    ...
    // Configure joystick debounce time from config if available
    if (config.sensors && config.sensors.joystickDebounceTime) {
        joystickDebounceTime = config.sensors.joystickDebounceTime;
    }
    console.log(`Joystick debounce time set to ${joystickDebounceTime}ms`);
    ...
    return {
        ...
        setJoystickDebounceTime: (time) => { joystickDebounceTime = time; }
    };
}
...
// Function to publish joystick events to MQTT with debouncing
function publishJoystickEvent(event) {
    // Apply debouncing to prevent duplicate events
    const now = Date.now();
    const eventKey = `${event.action}-${event.direction}`;
    
    // Skip if this is a duplicate event within the debounce time
    if (lastJoystickEvent[eventKey] && 
        (now - lastJoystickEvent[eventKey]) < joystickDebounceTime) {
        console.log(`Debounced duplicate joystick event: ${eventKey}`);
        return;
    }
    
    // Update the last event timestamp
    lastJoystickEvent[eventKey] = now;
