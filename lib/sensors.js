...
async function readSensorData() {
    return new Promise((resolve, reject) => {
        // Run a more comprehensive Python script to get accurate readings
        exec('python3 -c "from sense_hat import SenseHat; import time; s=SenseHat(); time.sleep(0.1); temp=s.get_temperature(); humidity=s.get_humidity(); pressure=s.get_pressure(); print(f\'{temp} {humidity} {pressure}\')"', (error, stdout, stderr) => {
            if (error) {
